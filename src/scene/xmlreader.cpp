...
#include <scene/materials/bxdfs/speculartransmission.h>
#include <scene/materials/bxdfs/fresnel_specularreflection.h>
...
#include <scene/materials/bxdfs/anisotropic_bxdf.h>
#include <QImage>
...
Material* XMLReader::LoadMaterial(QXmlStreamReader &xml_reader, const QStringRef &local_path, QMap<QString, QList<Material *> > &map)
{
    ...
    bool transmissive_material = false;
    ...
    else if(QStringRef::compare(type, QString("transmissivematerial")) == 0)
    {
        //std::cout << "here" << std::endl;
        result = new transmissivematerial();
        transmissive_material = true;
    }
    else if(QStringRef::compare(type, QString("cloud")) == 0)
    {
        result = new cloudmaterial();
    ...
    while(!xml_reader.isEndElement() || QStringRef::compare(xml_reader.name(), QString("material")) != 0)
    {
        ...
        else if (QString::compare(tag, QString("ni")) == 0 && transmissive_material) {
            xml_reader.readNext();
            float ni;
            if(xml_reader.isCharacters()){
                ni = xml_reader.text().toFloat();
            }
            ((transmissivematerial*)result)->ni = ni;
        }
        else if (QString::compare(tag, QString("nt")) == 0 && transmissive_material) {
            xml_reader.readNext();
            float nt;
            if(xml_reader.isCharacters()){
                nt = xml_reader.text().toFloat();
            }
            ((transmissivematerial*)result)->nt = nt;
        }
    }
    return result;
}
...
Camera XMLReader::LoadCamera(QXmlStreamReader &xml_reader)
{
    ...
    while(!xml_reader.isEndElement() || QStringRef::compare(xml_reader.name(), QString("camera")) != 0)
    {
        ...
        else if(QString::compare(tag, QString("enableDOF")) == 0)
        {
            xml_reader.readNext();
            if(xml_reader.isCharacters())
            {
                result->enableDOF = xml_reader.text().toFloat();
            }
            xml_reader.readNext();
        }
        ...
        else if(QString::compare(tag, QString("focalDistance")) == 0)
        {
            ...
            if(xml_reader.isCharacters())
            {
                result->focalDistance = xml_reader.text().toFloat();
            }
            xml_reader.readNext();
        }
    }
...
BxDF* XMLReader::LoadBxDF(QXmlStreamReader &xml_reader)
{
    ...
    else if(QStringRef::compare(type, QString("blinnMicrofacet")) == 0)
    {
        ...
        QStringRef ei = attribs.value(QString(), QString("ei"));
        if(QStringRef::compare(ei, QString("")) != 0) {
            ei_f = ei.toFloat();
        }
        QStringRef et = attribs.value(QString(), QString("et"));
        if(QStringRef::compare(ei, QString("")) != 0) {
            et_f = et.toFloat();
        }
        result = new speculartransmission(T,ei_f,et_f);
    }
    else if(QStringRef::compare(type, QString("fresnel_specularreflection"))==0){
        glm::vec3 R(0.5f);
        float ei_f = 1.0f;
        float et_f = 1.0f;
        glm::vec3 eta_v(0.5f);
        ...
        QStringRef eta = attribs.value(QString(),QString("eta"));
        if(QStringRef::compare(eta, QString("")) != 0){
            eta_v = ToVec3(eta);
        }
        QStringRef k = attribs.value(QString(),QString("k"));
        if(QStringRef::compare(k, QString("")) != 0){
            k_v = ToVec3(k);
        }
        QStringRef isDiel = attribs.value(QString(),QString("isDiel"));
        if(QStringRef::compare(isDiel, QString("")) != 0){
            is_Diel = isDiel.toFloat();
        }
        result = new fresnel_specularreflection(R,ei_f, et_f, eta_v, k_v, is_Diel);
    }
        ...
        float ey_f = 1.0f;
        float e_f = 1.0f;
        QStringRef color1 = attribs.value(QString(),QString("diffuseColor"));
        if(QStringRef::compare(color1, QString("")) != 0){
            color = ToVec3(color1);
        }
        QStringRef ex = attribs.value(QString(), QString("ex"));
        if(QStringRef::compare(ex, QString("")) != 0) {
            ex_f = ex.toFloat();
        }
        QStringRef ey = attribs.value(QString(), QString("ey"));
        if(QStringRef::compare(ey, QString("")) != 0) {
            ey_f = ey.toFloat();
        }
        QStringRef e = attribs.value(QString(), QString("e"));
        if(QStringRef::compare(e, QString("")) != 0) {
            e_f = e.toFloat();
        }
        result = new anisotropic_bxdf(color, ex_f, ey_f, e_f);
    }
    else
    {
