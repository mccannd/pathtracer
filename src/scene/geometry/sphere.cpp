...
// Get a sample point on a sphere, but doesn't take into about the normal to get you the closest side
Intersection Sphere::GetSamplePoint(const glm::vec3 &isx_normal) {
//    float u1 = 0.f; // NOT RAND
//    float u2 = 0.f;

    float u1 = qrand() / (float) RAND_MAX;
    float u2 = qrand() / (float) RAND_MAX;
...
Intersection Sphere::GetIntersection(Ray r)
{
    ...
    if(t >= 0)
    {
        ...
        result.object_hit = this;
        if ((fequal(P[0], 0.0f) && fequal(P[1], 0.5f) && fequal(P[0], 0.0f))
             || (fequal(P[0], 0.0f) && fequal(P[1], -0.5f) && fequal(P[0], 0.0f))) {
            result.tangent = glm::cross(glm::vec3(1, 0, 0), normal);
            result.bitangent = glm::cross(result.tangent, normal);
        } else {
            result.tangent = glm::cross(glm::vec3(0, 1, 0), normal);
            result.bitangent = glm::cross(result.tangent, normal);
        }

        result.tangent = glm::normalize(glm::vec3(transform.T() * glm::vec4(result.tangent, 0)));
        result.bitangent = glm::normalize(glm::vec3(transform.T() * glm::vec4(result.bitangent, 0)));
        return result;
    }
    return result;
}
