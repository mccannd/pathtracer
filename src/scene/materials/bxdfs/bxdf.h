...
//Member variables
    ...
    //pbrt pg 426
    inline float CosTheta(const glm::vec3 &w) const {return w.z;}
    inline float AbsCosTheta(const glm::vec3 &w) const {return fabsf(w.z);}
    inline float SinTheta2(const glm::vec3 &w) const {
        return fmax(0.f, 1.f - CosTheta(w)*CosTheta(w));
    }
    inline float SinTheta(const glm::vec3 &w) const {
        return sqrtf(SinTheta2(w));
    }
    inline float CosPhi(const glm::vec3 &w) const {
        float sintheta = SinTheta(w);
        if(sintheta == 0.f) return 1.f;
        return glm::clamp(w.x / sintheta, -1.f, 1.f);
    }
    inline float SinPhi(const glm::vec3 &w) const {
        float sintheta = SinTheta(w);
        if (sintheta == 0.f) return 0.f;
        return glm::clamp(w.y / sintheta, -1.f, 1.f);
    }

    //SameHemisphere pg 696
    inline bool SameHemisphere(const glm::vec3 &w, const glm::vec3 &wp) const {
        return w.z * wp.z > 0.f;
    }

    //SphericalDirection pg 292
    inline glm::vec3 SphericalDirection(float sintheta, float costheta, float phi) const {
