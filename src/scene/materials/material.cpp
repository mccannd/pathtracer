...
glm::vec3 Material::EvaluateScatteredEnergy(const Intersection &isx, const glm::vec3 &woW, const glm::vec3 &wiW, BxDF *bxdf_rand, BxDFType flags) const
{
    ...
    glm::vec3 ESE = isx.texture_color * base_color * bxdf_rand->EvaluateScatteredEnergy(wo, wi);
    return ESE;
}

// Remember to SET BxDF prior to SESE
glm::vec3 Material::SampleAndEvaluateScatteredEnergy(const Intersection &isx, const glm::vec3 &woW, glm::vec3 &wiW_ret, float &pdf_ret, BxDF *bxdf_rand, BxDFType flags) const
{
    // Convert to Normal Space
    glm::mat3 T = glm::mat3(isx.tangent, isx.bitangent, isx.normal);
    T = glm::transpose(T);
    ...
    // Random Sample
    glm::vec3 SARSE = isx.texture_color * base_color * bxdf_rand->SampleAndEvaluateScatteredEnergy(wo, wiW_ret, qrand()/(float)RAND_MAX, qrand()/(float)RAND_MAX, pdf_ret);
//    glm::vec3 SARSE = isx.texture_color * base_color * bxdf_rand->SampleAndEvaluateScatteredEnergy(wo, wiW_ret, 0.5f, 0.f, pdf_ret); // NOT RANDOM
...
BxDF* Material::GetRandomBxDF() const {
    // Since this isn't a weighted material with multiple BxDfs, return the first BxDF
    return bxdfs.at(0);
}
